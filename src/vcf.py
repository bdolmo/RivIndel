...
class VCFwriter():
    ...
    def __init__(self, vcf_out, template=None, compress=False, sample_name=None):
        ...
        if self._template is not None:
            ...
            header_title = ("#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t{}")\
    ...
    def add_info_to_header(self, info_dict):
        ...
        for line in header_list:
            ...
            if line.startswith("##INFO"):
                ...
                if seen is False:
                    for entry in info_list:
                        new_info = '##INFO=<ID={},Number={},Type={},Description={}>'\
                        .format(entry['ID'], entry['Number'], entry['Type'], entry['Description'])
    ...
    def close(self):
        ...
        self._fp.close()
        if self._compress is True:
            pysam.tabix_compress(self._vcf_out,
                self._vcf_gz_out, force=True)
            pysam.tabix_index(self._vcf_gz_out,
                preset="vcf", force=True)
...
class Record():
                        ...
                        for i in idx_dict:
                            ...
                            if 'Phastcons' in custom_key or 'PhyloP' in custom_key:
                                ...
                                if len(float_value_list) > 0:
                                    new_value = sum(float_value_list) / len(float_value_list)
    ...
    def SAMPLE(self) -> str:
        if len(self._variant_list) > 10:
            sample = '\t'.join([self._variant_list[i] for i in range(9, len(self._variant_list), 1)])
        else:
    ...
    def get_tier_classification(self, source):
        ...
        sensitive_significances = ["Sensitivity/Response", "Outcome",
            "Function"]
        ...
        for significance in self.clinical_significance:
            if "Sensitivity/Response" in significance:
                is_sensitive = True
            if "Outcome" in significance:
                is_sensitive = True
            if "Function" in significance:
                is_sensitive = True
        is_rare = False
        clinvar = "."
        impact = "."
        ...
        if is_rare is True:
            ...
            elif is_supporting is True and is_sensitive is False and is_druggable is True:
                clinical_classification = "Other"
            else:
                if self.vartype == "SV":
                    # require paired-end reads and split-reads
                    if self.discordant_reads[1] != "." and self.split_reads[1] != ".":
                        if int(self.discordant_reads[1]) > 3 and int(self.split_reads[1]) > 3:
                            clinical_classification = "Other"
